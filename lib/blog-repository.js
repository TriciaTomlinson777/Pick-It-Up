...
import { createPrivacySafePublicImage } from './future-photo-moderation';
...
function buildPostPayload(input, { statusOverride } = {}) {
  ...
  if (Array.isArray(input.submissionPhotoUrls)) {
    payload.submission_photo_urls = input.submissionPhotoUrls;
  }
  if (Array.isArray(input.submissionPhotoPaths)) {
    payload.submission_photo_paths = input.submissionPhotoPaths;
  }
...
async function uploadImageObject({ fileBuffer, contentType, fileName, folder = 'blog-images' }) {
  ...
  const publicImage = await createPrivacySafePublicImage(fileBuffer);
  const extension = publicImage.extension;
