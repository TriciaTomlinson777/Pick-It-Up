...
async function getStorageError(response) {
  let data;
  try {
  ...
  return {
    ...
    code: String(data?.code || data?.statusCode || '').slice(0, 80) || undefined,
    message: String(data?.message || data?.error || '').slice(0, 300) || undefined,
  };
}

function isMissingBucketError(storageError) {
  const code = String(storageError?.code || '').toLowerCase();
...
async function ensurePrivatePhotoBucket() {
  if (privateBucketPromise) return privateBucketPromise;

  privateBucketPromise = (async () => {
    const bucket = getPrivateBucketName();
    const encodedBucket = encodeURIComponent(bucket);
    const bucketResponse = await supabaseServerFetch(`/storage/v1/bucket/${encodedBucket}`);

    if (bucketResponse.ok) {
      const existingBucket = await bucketResponse.json();
      if (existingBucket?.public) {
        throw new Error(`Private photo bucket "${bucket}" must not be public.`);
      }
      return bucket;
    }

    const storageError = await getStorageError(bucketResponse);
    if (!isMissingBucketError(storageError)) {
      console.error('Unable to inspect private photo bucket.', storageError);
      throw new Error(`Private photo bucket check failed (${bucketResponse.status}).`);
    }

    const createResponse = await supabaseServerFetch('/storage/v1/bucket', {
      method: 'POST',
    ...
    if (!createResponse.ok) {
      const storageError = await getStorageError(createResponse);
      console.error('Unable to create private photo bucket.', storageError);
      throw new Error(`Private photo bucket creation failed (${createResponse.status}).`);
    }

    return bucket;
  })().catch((error) => {
    privateBucketPromise = undefined;
    throw error;
  });

  return privateBucketPromise;
}
...
export async function storeAndModerateFutureImage(file, folder, submissionId, index = 0) {
  ...
  const bucket = await ensurePrivatePhotoBucket();
  const encodedBucket = encodeURIComponent(bucket);
  ...
  if (!uploadResponse.ok) {
    const storageError = await getStorageError(uploadResponse);
    console.error('Private image upload failed.', storageError);
