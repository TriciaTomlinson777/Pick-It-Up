...
function getSecret() {
  ...
  console.log(DIAG, 'session-secret presence check passed');
  return secret;
}
...
export function validateAdminCredentials(username, password) {
  ...
  console.log(DIAG, 'credential match result', { usernameMatch, passwordMatch });

  return usernameMatch && passwordMatch;
}

export function getAdminSessionCookieConfig({ expires, maxAge } = {}) {
  const config = {
    httpOnly: true,
  ...
  if (typeof maxAge === 'number') {
    ...
    config.maxAge = DEFAULT_SESSION_HOURS * 60 * 60;
  }
