...
import { createSignedPhotoUrl, isFutureUploadPath } from './future-photo-moderation';
...
async function normalizeRow(type, row) {
  const imageUrl = String(row.image_url || '') || (isFutureUploadPath(row.image_path) ? await createSignedPhotoUrl(row.image_path) : '');
  const beforeImageUrl = String(row.before_image_url || '') || (isFutureUploadPath(row.before_image_path) ? await createSignedPhotoUrl(row.before_image_path) : '');
  const afterImageUrl = String(row.after_image_url || '') || (isFutureUploadPath(row.after_image_path) ? await createSignedPhotoUrl(row.after_image_path) : '');
  return {
    type,
    id: String(row.id || ''),
...
export async function getAdminCommunityContent() {
  const sections = await Promise.all(
    Object.values(CONTENT_TYPES).map(async (config) => {
      ...
      const normalizedItems = await Promise.all((Array.isArray(rows) ? rows : []).map((row) => normalizeRow(config.key, row)));
      return {
        key: config.key,
        title: config.title,
...
export async function updateAdminCommunityContentStatus(type, id, status) {
  ...
  const query = createQueryString({
    id: `eq.${id}`,
    moderation_status: 'eq.pending_review',
    select: config.select,
  });
...
export async function deleteAdminCommunityContent(type, id) {
  const config = getContentConfig(type);
  const query = createQueryString({ id: `eq.${id}`, moderation_status: 'eq.pending_review' });
