...
export async function POST(request) {
  ...
  try {
    ...
    if (!savedProfile) throw new Error('Participant profile row was not found after avatar update.');

    const matchesRequest = savedProfile.id === user.id
      && savedProfile.avatar_kind === kind
      && (savedProfile.avatar_preset || null) === (kind === 'preset' ? preset : null)
      && (savedProfile.avatar_path || null) === avatarPath
      && savedProfile.avatar_moderation_status === moderationStatus;
    if (!matchesRequest) throw new Error('Participant profile avatar did not match the saved request.');
