...
const ALLOWED_POST_FIELDS = new Set(['caption', 'image_url', 'image_path']);
...
export async function GET() {
  try {
    ...
    if (!response.ok) {
      ...
      console.error('Failed to load scenic discoveries.', supabaseMessage || response.status);
      return Response.json(
        { error: 'Unable to load scenic discoveries.' },
        { status: 500 }
      );
    }
...
export async function POST(request) {
  ...
  try {
    ...
    return Response.json({
      ...
      image_path: uploaded?.storagePath || null,
      image_url: uploaded?.storagePath && createdRow.moderation_status === 'approved'
        ? await createSignedPhotoUrl(uploaded.storagePath)
        : null,
    });
  } catch (error) {
